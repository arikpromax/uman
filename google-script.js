...
/* Кому слати замовлення. Можна один чат, можна кілька — через кому:
     const CHAT = ['677078927', '-1001234567890'];
   Людина побачить повідомлення тільки якщо сама написала боту
   хоч раз: без цього Telegram не дозволяє йому писати першим.
   Для групи достатньо додати туди бота й написати в ній будь-що. */
const CHAT  = '';
...
function doPost(e) {
  ...
  if (TOKEN && CHAT && CHAT.length) {
    ...
    [].concat(CHAT).forEach(function (id) {
      if (id) tg('sendMessage', { chat_id: id, text: text, parse_mode: 'HTML' });
    });
  }
